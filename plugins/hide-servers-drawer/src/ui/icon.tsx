 ...
 *
 * Icon naming splits in two: older icons are *registry assets* resolved by
 * `getAssetIdByName`, while Discord's newer set (the `redesign/generated/*.tsx` modules)
 * are generated React *components* that only `lookupGeneratedIconComponent` can see. Both
 * are tried, in that order, for every name given. First hit wins; if every name misses,
 * the row simply gets no icon rather than a broken image.
 *
