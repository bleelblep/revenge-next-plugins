 ...
 *
 * Copied from Translate. Rows cross into native once and live there afterwards, so mutating what `RowManager.generate`
...
/** Newest first, and bounded: a rule change only has to fix what is on or near the screen. */
const CHANNEL_REPAINT_LIMIT = 100

/**
 * Redraws the loaded messages of one channel, after a rule that affects it changed. Each one goes
 * through the same single-message path, so the rewrite still happens only in the row hook.
 */
export function repaintChannel(channelId: string): number {
	let messages: any[] = []
	try {
		const store = (revenge.discord.flux.Stores as any)?.MessageStore
		const loaded = store?.getMessages?.(channelId)
		messages = loaded?.toArray?.() ?? loaded?._array ?? []
	} catch {
		return 0
	}
	let count = 0
	for (const message of messages.slice(-CHANNEL_REPAINT_LIMIT)) {
		if (typeof message?.id === 'string' && repaintMessage(channelId, message.id)) count++
	}
	debug(`repainted ${count} messages in ${channelId}`)
	return count
}
